...
var supportsScriptLoad = (common_js_1.isBrowser || common_js_1.isWorker) && typeof navigator !== 'undefined' && navigator.userAgent && !navigator.userAgent.match(/MSIE (9|10).0/);
if (typeof require !== 'undefined' && typeof process !== 'undefined' && !process.browser)
    exports.nodeRequire = require;
...
function instantiate(key, processAnonRegister) {
    var loader = this;
    var config = this[common_js_1.CONFIG];
    return (loadBundlesAndDepCache(config, this, key) || common_js_1.resolvedPromise)
        .then(function () {
        if (processAnonRegister())
            return;
        var metadata = loader[common_js_1.METADATA][key];
        if (key.substr(0, 6) === '@node/') {
            if (!loader._nodeRequire)
                throw new TypeError('Error loading ' + key + '. Can only load node core modules in Node.');
            loader.registerDynamic([], false, function () {
                return common_js_1.loadNodeModule.call(loader, key.substr(6), loader.baseURL);
            });
            processAnonRegister();
            return;
        }
        if (metadata.load.scriptLoad) {
            if (metadata.load.pluginKey || !supportsScriptLoad) {
                ...
                common_js_1.warn.call(config, 'scriptLoad not supported for "' + key + '"');
            }
        }
        ...
        return new Promise(function (resolve, reject) {
            if (metadata.load.format === 'amd' && common_js_1.global.define !== loader.amdDefine)
                throw new Error('Loading AMD with scriptLoad requires setting the global `' + globalName + '.define = SystemJS.amdDefine`');
            common_js_1.scriptLoad(key, metadata.load.crossOrigin, metadata.load.integrity, function () {
                if (!processAnonRegister()) {
                    metadata.load.format = 'global';
                    var globalValue = metadata.load.exports && format_helpers_js_1.getGlobalValue(metadata.load.exports);
                    loader.registerDynamic([], false, function () {
        ...
        delete loader[common_js_1.METADATA][key];
        return instantiated;
    });
}
...
function loadBundlesAndDepCache(config, loader, key) {
    var deps;
    if (common_js_1.isBrowser && (deps = config.depCache[key])) {
        for (var i = 0; i < deps.length; i++)
            loader.normalize(deps[i], key).then(common_js_1.preloadScript);
    }
    else {
        var matched = false;
...
function runFetchPipeline(loader, key, metadata, processAnonRegister, wasm) {
    ...
    return common_js_1.resolvedPromise
        ...
        if (!metadata.pluginModule)
            return fetch_js_1.default(key, metadata.load.authorization, metadata.load.integrity, wasm);
        wasm = false;
        if (!metadata.pluginModule.fetch)
            return fetch_js_1.default(metadata.pluginLoad.address, metadata.load.authorization, metadata.load.integrity, false);
        return metadata.pluginModule.fetch.call(loader, metadata.pluginLoad, function (load) {
            return fetch_js_1.default(load.address, metadata.load.authorization, metadata.load.integrity, false);
        });
    })
        .then(function (fetched) {
        if (!wasm || typeof fetched === 'string')
            return translateAndInstantiate(loader, key, fetched, metadata, processAnonRegister);
        return common_js_1.checkInstantiateWasm(loader, fetched, processAnonRegister)
            .then(function (wasmInstantiated) {
            if (wasmInstantiated)
                return;
            var stringSource = common_js_1.isBrowser ? new TextDecoder('utf-8').decode(new Uint8Array(fetched)) : fetched.toString();
...
function translateAndInstantiate(loader, key, source, metadata, processAnonRegister) {
    return Promise.resolve(source)
        ...
        if (metadata.load.format !== 'esm' && (metadata.load.format || !source.match(exports.esmRegEx))) {
            return source;
        }
        ...
        return Promise.resolve(metadata.pluginModule.instantiate.call(loader, metadata.pluginLoad, function (load) {
            ...
            if (calledInstantiate)
                return translated;
            return common_js_1.protectedCreateNamespace(result);
        });
    })
        .then(function (source) {
        ...
        switch (metadata.load.format) {
            ...
            case 'register':
            case 'system':
                var err = evaluate_js_1.evaluate(loader, source, metadata.load.sourceMap, key, metadata.load.integrity, metadata.load.nonce, false);
                if (err)
                    throw err;
                if (!processAnonRegister())
                    return common_js_1.emptyModule;
                return;
                break;
            case 'json':
            ...
            case 'amd':
                ...
                common_js_1.global.define = loader.amdDefine;
                format_helpers_js_1.clearLastDefine(metadata.load.deps, metadata.load.esModule);
                var err = evaluate_js_1.evaluate(loader, source, metadata.load.sourceMap, key, metadata.load.integrity, metadata.load.nonce, false);
            ...
            case 'cjs':
                ...
                var deps = (metadata.load.deps || []).concat(metadata.load.cjsRequireDetection ? format_helpers_js_1.getCJSDeps(source) : []);
                ...
                loader.registerDynamic(deps, true, function (require, exports, module) {
                    require.resolve = function (key) {
                        return format_helpers_js_1.requireResolve.call(loader, key, module.id);
                    };
                    module.paths = [];
                    ...
                    if (!metadata.load.cjsDeferDepsExecute && metaDeps)
                        for (var i = 0; i < metaDeps.length; i++)
                            require(metaDeps[i]);
                    var pathVars = format_helpers_js_1.getPathVars(module.id);
                    var __cjsWrapper = {
                        exports: exports,
                        args: [require, exports, module, pathVars.filename, pathVars.dirname, common_js_1.global, common_js_1.global]
                    };
                    ...
                    var define = common_js_1.global.define;
                    common_js_1.global.define = undefined;
                    common_js_1.global.__cjsWrapper = __cjsWrapper;
                    source = cjsWrapper + ") {" + source.replace(hashBangRegEx, '') + "\n}).apply(__cjsWrapper.exports, __cjsWrapper.args);";
                    var err = evaluate_js_1.evaluate(loader, source, metadata.load.sourceMap, key, metadata.load.integrity, metadata.load.nonce, false);
                    if (err)
                        throw err;
                    setMetaEsModule(metadata, exports);
                    common_js_1.global.__cjsWrapper = undefined;
                    common_js_1.global.define = define;
                });
                registered = processAnonRegister();
                break;
            case 'global':
                ...
                loader.registerDynamic(deps, false, function (require, exports, module) {
                    ...
                    var retrieveGlobal = format_helpers_js_1.prepareGlobal(module.id, exportName, globals, metadata.load.encapsulateGlobal);
                    var err = evaluate_js_1.evaluate(loader, source, metadata.load.sourceMap, key, metadata.load.integrity, metadata.load.nonce, true);
                    if (err)
                        throw err;
...
exports.detectRegisterFormat = detectRegisterFormat;
...
function detectLegacyFormat(source) {
    ...
    common_js_1.cjsRequireRegEx.lastIndex = 0;
    if (common_js_1.cjsRequireRegEx.exec(source) || cjsExportsRegEx.exec(source))
        return 'cjs';
    return 'global';
}
exports.detectLegacyFormat = detectLegacyFormat;
...
function transpile(loader, source, key, metadata, processAnonRegister) {
    ...
    return loader.import.call(loader, loader.transpiler)
        ...
        throw common_js_1.addToError(err, 'Unable to load transpiler to transpile ' + key);
    });
}
var metaRegEx = /^(\s*\/\*[^\*]*(\*(?!\/)[^\*]*)*\*\/|\s*\/\/[^\n]*|\s*"[^"]+"\s*;?|\s*'[^']+'\s*;?)+/;
var metaPartRegEx = /\/\*[^\*]*(\*(?!\/)[^\*]*)*\*\/|\/\/[^\n]*|"[^"]+"\s*;?|'[^']+'\s*;?/g;
